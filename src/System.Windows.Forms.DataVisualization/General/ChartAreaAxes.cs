...
#pragma warning restore CA2213 // Disposable fields should be disposed
    ...
    /// </summary>
    internal List<string> ChartTypes => chartTypes;
    ...
    private void SetDefaultFromData(Axis axis)
    {
            ...
            ((axis.AutoMinimum || double.IsNaN(axis.Minimum)) && (autoMinimum == double.MaxValue || autoMinimum == double.MinValue)))
        {
            ...
            {
                // Suppress exception and use predefined min & max
                autoMaximum = 8.0;
                autoMinimum = 1.0;
            }
            else
        ...
        // Do not make axis margin for logarithmic axes
        if (axis.IsLogarithmic)
        {
            axis.marginView = 0.0;
        }
        ...
        else // Y axis type is used (Y or Y2)
        {
            ...
            if (stacked) // Chart area has a stacked chart types
            {
                try
                {
                    ...
                    {
                        // If stacked groups are used Min/Max range must calculated
    ...
    private void SetDefaultFromIndexes(Axis axis)
    {
        ...
        // Find the interval. If the number of points
    ...
    {
        return GetPointsInterval(Series, isLogarithmic, logarithmBase, false, out _);
    }
    ...
    {
        return GetPointsInterval(seriesList, isLogarithmic, logarithmBase, checkSameInterval, out sameInterval, out _);
    }
    ...
    internal double GetPointsInterval(List<string> seriesList, bool isLogarithmic, double logarithmicBase, bool checkSameInterval, out bool sameInterval, out Series series)
    {
        ...
        if (checkSameInterval && !sameInterval && seriesXValues.Length > 1)
        {
            ...
            // Use side-by-side if at least one common X value between Series found
            if (sameXValue)
            {
                sameInterval = true;
            }
