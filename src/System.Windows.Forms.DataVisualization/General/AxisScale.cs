...
#pragma warning restore CA2213 // Disposable fields should be disposed
    ...
    public double Maximum
    {
        ...
        set
        {
            ...
            this.tempAutoMaximum = AutoMaximum;
...
DesignerSerializationVisibility(DesignerSerializationVisibility.Content),
    ...
    internal double ViewMinimum => _scaleView.ViewMinimum;

    /// <summary>
    /// Gets axis data scaleView minimum position.
    /// </summary>
    /// <returns>Axis data scaleView minimum position.</returns>
    internal double ViewMaximum => _scaleView.ViewMaximum;
    ...
    /// </summary>
    internal bool AutoMinimum => _autoMinimum;
    ...
    internal double PositionToValue(double position, bool validateInput)
    {
        ...
        if (AxisPosition == AxisPosition.Top || AxisPosition == AxisPosition.Bottom)
        {
            if ((Parent as ChartArea)?.ReverseSeriesOrder == true) // we rotated the chart by more then 90 degrees in 3DMode
                position = PlotAreaPosition.Right - position;
            else
                position -= PlotAreaPosition.X;

            ChartArea = PlotAreaPosition.Width;
        }
        else // we can't rotate by more then 90 degrees around X axis, so no ReverseSeriesOrder check here
        {
    ...
    private double GetCrossing()
    {
        ...
        {
            if (Common.ChartTypeRegistry.GetChartType(ChartArea.ChartTypes[0]).ZeroCrossing)
            {
                if (ViewMinimum > 0.0)
                {
                    return ViewMinimum;
    ...
    internal void EstimateAxis()
    {
        ...
        if (!double.IsNaN(_scaleView.Position) && !double.IsNaN(_scaleView.Size))
        {
            ...
            else
            {
                ...
                EstimateAxis(ref this.minimum, ref this.maximum, AutoMaximum, _autoMinimum);
            }
        ...
        else // No Zooming mode
        {
            ...
            axisInterval = EstimateAxis(ref this.minimum, ref this.maximum, AutoMaximum, _autoMinimum);
        }
