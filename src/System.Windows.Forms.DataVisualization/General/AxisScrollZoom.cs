...
namespace System.Windows.Forms.DataVisualization.Charting
{
    ...
    public class AxisScaleView
    {
		...
		/// </summary>
		/// <param name="viewPosition">New data scaleView start position.</param>
        ...
        private void GetCurrentViewSmallScrollSize()
        {
            ...
            if (double.IsNaN(_currentSmallScrollSize))
            {
                ...
                if (this.SizeType == DateTimeIntervalType.Auto || this.SizeType == DateTimeIntervalType.Number)
                {
                    ...
                    // Divide scaleView by 20 to find the scrolling line size
        ...
        {
            // Scroll line size/type is specifically set by user
        ...
        private void ValidateViewPositionSize(ref double viewPosition, ref double viewSize, ref DateTimeIntervalType viewSizeType)
        {
            ...
            while (viewPosition + newViewSize > axis.maximum)
            {
                ...
                if (newViewSize > minViewSize)
                {
                    ...
                    if (newViewSize < minViewSize)
                    {
                        // Can't adjust size no more (restore previous value)
    ...
    internal sealed class DoubleDateNanValueConverter : DoubleConverter
    {
		...
		public override object ConvertTo(ITypeDescriptorContext context, CultureInfo culture, object value, Type destinationType)
        {
            ...
            if (context != null && context.Instance != null)
            {
                ...
                Axis axis = null;
                if (context.Instance is AxisScaleView view)
                {
                    axis = view.axis;
                }
                else if (context.Instance is Cursor cursor)
                {
                    axis = cursor.GetAxis();
                }
