...
internal sealed class SplineRangeChart : RangeChart
{
    ...
    /// Chart type name
    /// </summary>
    public override string Name => ChartTypeNames.SplineRange;
...
internal class RangeChart : SplineChart
{
    ...
    /// Chart type name
    /// </summary>
    public override string Name => ChartTypeNames.Range;
        ...
        float tension)
    {
        ...
        if (common.ProcessModeRegions)
        {
            ...
            using GraphicsPath mapAreaPath = new GraphicsPath();
            ...
            (firstPoint.yPosition < thirdPoint.Y && secondPoint.yPosition > fourthPoint.Y)))
        {
            ...
            if (splitDraw)
            {
                ...
                for (int segmentIndex = 0; segmentIndex <= 1; segmentIndex++)
                {
                    ...
                    // Add segment path
                    if (resultPath is not null && segmentPath is not null && segmentPath.PointCount > 0)
...
#pragma warning restore CA2000 // Dispose objects before losing scope
                ...
                if (elemLayer == 2 && resultPath is not null && surfacePath is not null && surfacePath.PointCount > 0)
                {
        ...
        ref SurfaceNames visibleSurfaces)
    {
        ...
        if (!area.ReverseSeriesOrder)
        {
            if ((!upSideDown && firstPoint.xPosition < secondPoint.xPosition) ||
                (upSideDown && firstPoint.xPosition > secondPoint.xPosition))
            {
                cubePoints[0] = new Point3D((float)firstPoint.xPosition, thirdPoint.Y, positionZ + depth);
                cubePoints[1] = new Point3D((float)firstPoint.xPosition, thirdPoint.Y, positionZ);
                cubePoints[2] = new Point3D((float)secondPoint.xPosition, fourthPoint.Y, positionZ);
            }
            else
            {
                cubePoints[0] = new Point3D((float)secondPoint.xPosition, fourthPoint.Y, positionZ + depth);
                cubePoints[1] = new Point3D((float)secondPoint.xPosition, fourthPoint.Y, positionZ);
                cubePoints[2] = new Point3D((float)firstPoint.xPosition, thirdPoint.Y, positionZ);
            }
        }
        else
        {
        ...
        bool forceThinBorder)
    {
        ...
        // Get top line path
        using GraphicsPath topLine = graph.GetSplineFlattenPath(
            area, positionZ,
            firstPoint, secondPoint, points, tension, false, true, 0);

        // Get bottom line path
        using GraphicsPath bottomLine = graph.GetSplineFlattenPath(
            area, positionZ,
        ...
        if (drawElements)
        {
            ...
            if (forceThinBorder)
            {
                using var pen = new Pen(surfaceBorderColor, 1);
                graph.DrawPath(pen, resultPath);
            }
        ...
        if (resultPath is not null && thickBorderPen is not null)
        {
            // Widen result path
            try
            {
